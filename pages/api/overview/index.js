...
// local month start from "YYYY-MM" (or current month)
...
function monthRangeLocal(key) {
  const start = monthStartLocal(key);
...
// Accept occupancy as 0..1 or 0..100
const pct = (v) => {
  const n = Number(v);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const dailySeries = rows.map((r) => {
      ...
      return {
        day: new Date(r.date).getUTCDate(), // day number (1..31)
    ...
    // ---- room types for month (aggregate) ----
    ...
    const history = await prisma.yearMetric
      ...
      .then((ys) =>
        ys.map((y) => ({
          ...
          occupancy: Math.round(pct(y.occupancy) * 10) / 10, // normalize
